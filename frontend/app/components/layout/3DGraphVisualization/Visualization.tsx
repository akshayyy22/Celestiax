...
const GraphVisualization: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedCrypto = sessionStorage.getItem("selectedCrypto");
      if (storedCrypto) {
  ...
  const renderGraph = (updatedGraphData: any) => {
    ...
    const batchSize = 500; // Number of nodes/links to add in each batch
    const delay = 900; // Delay in milliseconds between batches
